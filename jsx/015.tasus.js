...
const updateTasus = (cpy, bal, ste) => {
    ...
    exec('tsc -b 015.tasus', async (err, stdout, stderr) => {
        if (err) {
            console.error(`exec error: ${err}`);
        }
        process.chdir("../999.vurt");
        bit = await ste.bus(ActVrt.BUNDLE_VURT, { src: "015.tasus" });
        process.chdir("../015.tasus");
        bit = await ste.bus(ActDsk.READ_DISK, { src: './work/015.tasus.js' });
        var tasus = bit.dskBit.dat;
        bit = await ste.bus(ActDsk.WRITE_DISK, { src: '../gillisse/public/jsx/015.tasus.js', dat: tasus });
...
const editTasus = (cpy, bal, ste) => {
    ...
    exec('start Code.exe ../packages/gillisse', async (err, stdout, stderr) => {
        ...
        process.chdir("../packages/015.tasus");
        if (bal.slv != null)
...
const cloudTasus = async (cpy, bal, ste) => {
    ...
    bit = await ste.bus(ActDsk.WRITE_DISK, { src: './cloud/015.tasus.js', dat: space });
    bit = await ste.bus(ActDsk.COPY_DISK, { src: './cloud/', idx: '../../agent/015.tasus/' });
    const { exec } = require('child_process');
    process.chdir("../../agent/015.tasus");
